...
Usage:
    ...
    gauntlet demo
    gauntlet replay results.json
    gauntlet export --format json --output results.json
"""
...
@click.group()
@click.version_option(version="1.0.0", prog_name="gauntlet")
def cli():
...
def stats(scenarios):
    ...
    from gauntlet.core import ScenarioLoader
...
def serve(host, port, debug):
    ...
    try:
        from gauntlet.api import create_app
    except ImportError:
        click.echo("Flask is required for the API server.")
        click.echo("Install with: pip install gauntlet[flask]")
        sys.exit(1)

    app = create_app()
    click.echo(f"Starting Gauntlet API on {host}:{port}")
...
def demo(port):
    ...
    click.echo("Starting Gauntlet in demo mode...")

    # Start mock target
    from gauntlet.demo.mock_target import create_mock_app
    ...
    # Seed fixture data
    from gauntlet.demo import load_fixtures
    ...
    try:
        ...
        click.echo("Flask is required. Install with: pip install gauntlet[flask]")
        sys.exit(1)
