"""
Gauntlet Red Team -- Multi-Provider Comparison
================================================
...
Note: This module requires ``sentinel`` to be installed
(``from sentinel import Shield, create_llm_judge``).  The
imports are function-scoped and only trigger at runtime.

Usage:
    python -m gauntlet.providers.multi_provider                       # All detected providers
    python -m gauntlet.providers.multi_provider --providers openai    # Specific subset
    python -m gauntlet.providers.multi_provider --scenarios s.yaml    # Custom YAML scenarios

Requires:
    pip install sentinel python-dotenv
...
def main():
    parser = argparse.ArgumentParser(
        description="Gauntlet Shield Multi-Provider Red Team Comparison"
    )
    parser.add_argument(
        "--providers",
    ...
    print("=" * 70)
    print("GAUNTLET SHIELD -- MULTI-PROVIDER RED TEAM COMPARISON")
    print("=" * 70)
